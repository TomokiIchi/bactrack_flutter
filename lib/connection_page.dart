...
class _ConnectionPageState extends State<ConnectionPage> {
  ...
  Future _initPlugin(String apiKey) async {
    try {
      ...
      setState(
          () => _initStatus = StatusMessage(_initializingPluginText, true));
    } on PlatformException catch (e) {
      setState(() {
        _initStatus = StatusMessage(_initializingPluginText, false,
            "$_initializationErrorText ${e.message}");
      });
    }
  }
}
...
class _ConnectionContainerState extends State<ConnectionContainer> {
  ...
  void initState() {
    ...
    _sub = widget.bacTrackPlugin.statusStream
        .map((status) {
          print(
              "Got status from plugin: ${status.state} with message: '${status.message}'");
          return status;
        })
          ...
          (status) {
            switch (status.state) {
              ...
              case BACtrackState.connectionTimeout:
                setState(() {
                  _connectionStatus =
                      StatusMessage(_connectToNearestDeviceText, false);
              ...
              case BACtrackState.didConnect:
                setState(() => _statuses
                    .add(StatusMessage(status.state.toString(), true)));
                break;
              ...
              case BACtrackState.connected:
                setState(() {
                  _isConnected = true;
                  _connectionStatus =
                      StatusMessage(_connectToNearestDeviceText, true);
                  _statuses.add(StatusMessage(status.state.toString(), true));
                });
                break;

              default:
                setState(() => _statuses.add(StatusMessage(
                    "$_unexpectedStateText ${status.state.toString()}",
                    false)));
            }
          },
        );
        ...
        if (_isConnected)
          OutlinedButton(
            onPressed: () => Navigator.of(context)
                .pushNamed(controlRoute, arguments: widget.bacTrackPlugin),
  ...
  void _handleConnectToNearest() {
    setState(() =>
        _connectionStatus = StatusMessage(_connectToNearestDeviceText, null));
